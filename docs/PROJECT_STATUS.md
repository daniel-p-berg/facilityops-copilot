...
- The structurally validated package expects 41 deliveries, 41 source-native records, 39 source-event groups, 40 logical variants, 76 canonical observations across all 28 topology point definitions, 82 lineage edges, 5 declared register decode groups, 1 exact-redelivery group, 1 conflicting-redelivery group, 1 missing-source-time record, 1 invalid-source-time record, no decode issue, and 1 asserted-action annotation.
- Structural package validation is allowlist-only and bounded. It enforces an explicit 4,000,000-byte repository-package limit, a 1,000,000-byte per-file limit, and a 65,536-byte per-delivery payload limit. It rejects arbitrary paths, archives, URLs, uploads, unregistered mappings, identity/digest mismatches, unresolved facility, source, channel, topology, mapping, package, delivery, replay-event, and oracle references, malformed time/identity structures, and prohibited evidentiary claims before persistence.
...
- The package is structurally validated before replay data is canonicalized and the complete in-memory plan is checked against the structural oracle. One explicit transaction publishes that plan; schema or replay fault injection rolls back without exposing a partial execution.
