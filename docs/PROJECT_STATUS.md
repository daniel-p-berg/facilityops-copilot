...
The 2026-07-23 canonical-observation and synthetic-replay verification used Python 3.12.13, FastAPI 0.136.3, Starlette 1.2.1, Pydantic 2.13.4, and Uvicorn 0.49.0. `.venv/bin/python -m unittest discover -s tests` ran all 363 discovered tests successfully in 6.605 seconds. The 80 focused topology, identity, temporal, canonicalization, lineage, projection, store, replay-package, API, and workbench tests passed in 1.959 seconds. Two separate executions of the structurally validated package produced normalized semantic digest `88703f59a062e349b1d3765909275bfebc6062d3f1846c7958677fa808888ec2`. Documentation review checked 126 local Markdown links and all 5 local anchors without error. After those checks, the default observation store remained absent, and the ignored operational database retained SHA-256 `57742e8873cdcb23f2b421b5aa7e017812919ef32fdd67ee1071c7f65cc952da`, size 319,488 bytes, and modification time `2026-07-22T21:47:49+0700`.

The 2026-07-23 observation-and-replay acceptance-correction verification used Python 3.12.13, FastAPI 0.136.3, Starlette 1.2.1, Pydantic 2.13.4, Uvicorn 0.49.0, and AnyIO 4.13.0. The bounded application import passed in 0.297 seconds. All 371 discovered tests passed; the test body completed in 6.827 seconds and the bounded process in 7.074 seconds. The 88 focused observation-topology, semantics, store, replay-package, API, workbench, terminology, and validation tests passed separately in 2.220 seconds. Four targeted malformed-package, fault-injected rollback, bitemporal projection rebuild, and input-order rebuild tests passed in 0.209 seconds. Two executions against one temporary store produced the same normalized semantic digest, `4eab13accadd357deca945d970d53e2d34f97d21719ee1d961617b8be1d81bdd`, from replay package digest `10ca39fe3d98553ee23fc6da46b4064f696c18e9416a794e7221e8a380f4d103`. Documentation review checked 126 local links across 28 tracked Markdown files, including all 5 local anchors, without error. ADRs 0001–0004, topology and standards packages `1.0.0`, Northstar fixtures, and alarm surfaces matched the starting commit byte-for-byte. All 24 requirement records across both standards packages remained inactive and non-executable. The default observation store and sidecars remained absent. The ignored operational database retained SHA-256 `57742e8873cdcb23f2b421b5aa7e017812919ef32fdd67ee1071c7f65cc952da`, size 319,488 bytes, and modification time `2026-07-22T21:47:49+0700`.

The 2026-07-24 observation-replay invariant-maintenance verification used the same recorded Python 3.12.13 dependency set. The bounded application import passed in 0.268 seconds; all 381 discovered tests passed in 7.771 seconds of test time and 8.012 seconds of bounded process time. The complete 381-test suite also passed with warnings treated as errors in 7.931 seconds. The 92-test affected-module selection covering observation semantics, replay-package and replay-plan validation, observation-store integrity, and API/workbench behavior passed with warnings treated as errors in 3.480 seconds. Added coverage verifies frontier ordering under sequence/time disagreement, same-sequence missing-time handling, cross-session register-pair rejection, composable distinct-event and exact-redelivery identity facts, exact narrative/action boundaries, usable mapping-normalization parameters, two-phase structural-oracle validation, mapping and decode-issue persistence scope, atomic rejection, and bounded SQLite pagination. Python compilation, inline JavaScript syntax, and `git diff --check` passed. Documentation review checked 126 local links across 28 tracked Markdown files, including all 5 local anchors, without error.

Preflight and postflight checks confirmed that the ignored local databases did not change during this maintenance pass. `db/facilityops.sqlite3` retained SHA-256 `5943c3201cf72e5fffad4288ef320165ec6fde0036833ae9eb73a294654d105e`, size 319,488 bytes, and modification time `2026-07-23T22:09:31+0700`. `db/facilityops-observations.sqlite3` retained SHA-256 `438ae60ef23cf0aa00af0b87057bf57c1aef7ecda0d66812f8349916534b7efe`, size 851,968 bytes, and modification time `2026-07-23T22:09:58+0700`. Read-only integrity and foreign-key checks passed for both files. The existing observation store still contains one execution with 41 deliveries, 41 source-native records, 76 canonical observations, 82 lineage edges, no decode issue, and normalized semantic digest `4eab13accadd357deca945d970d53e2d34f97d21719ee1d961617b8be1d81bdd`; these facts establish only the preserved local laboratory state.
...
- Its 20-entry narrative contains received-indication groups and one non-authoritative asserted-action annotation. Package context is pinned separately in the manifest. Event names and the structural oracle assert no fan failure, successful standby changeover, pressure-cascade or containment state, conformance, consequence, authorization, safety, or recovery result. Recovery evaluation, findings, and human disposition are not implemented.
...
- The package's structural oracle expects 41 deliveries, 41 source-native records, 39 source-event groups, 40 logical variants, 76 canonical observations across all 28 topology point definitions, 82 lineage edges, 5 declared register decode groups, 1 exact-redelivery group, 1 conflicting-redelivery group, 1 missing-source-time record, and 1 invalid-source-time record. The accepted baseline plan derives no decode issue, and the approved narrative inventory yields one asserted-action annotation.
- Pre-plan package validation is allowlist-only and bounded. It enforces an explicit 4,000,000-byte repository-package limit, a 1,000,000-byte per-file limit, and a 65,536-byte per-delivery payload limit. It verifies syntax, content digests, exact topology/mapping/source pins and references, the approved 20-event narrative inventory and ordinal order, observation-group-only delivery references, non-overlapping typed Boolean tokens, non-zero decimal factors, positive decimal quanta, and the supported structural-oracle schema. It rejects arbitrary paths, archives, URLs, uploads, unregistered mappings, malformed time/identity structures, and prohibited evidentiary claims. It does not claim that the oracle is true of derived replay rows.
...
- The store enables SQLite foreign keys and recursive triggers, enforces typed-value and identity constraints, prevents accepted-record update or deletion, and bounds payload and list-page sizes. Insert guards keep source-native and canonical mappings inside the replay topology, facility, and source-binding scope and keep decode issues inside the replay, topology, facility, mapping, and referenced-native-record scope. Page offsets beyond SQLite's signed-integer bound are rejected before query execution.
- After deterministic canonicalization, the complete in-memory plan is checked semantically against every declared identity group, decode-lineage expectation, ordering fact, projection expectation, and aggregate count. An oracle mismatch prevents publication. One explicit transaction then publishes the validated plan; a persistence scope violation or injected transaction fault rolls back without exposing a partial execution.
...
- Facility-nested routes catalog and inspect the one allowlisted replay package and create an isolated execution from package ID/version plus a request idempotency key; the API accepts no arbitrary package path. Package detail reports `package_validation = SYNTAX_AND_REFERENCES_VALID` and `structural_validation = REPLAY_PLAN_NOT_EVALUATED` because the structured semantic-oracle expectations are evaluated only while building an execution plan.
...
- The synthetic observation replay workbench shows the package-validation and replay-plan-validation split before execution. It lets a reviewer select the syntax/reference-validated package, inspect package/topology/mapping/canonicalizer identity and structural oracle, start a local execution that performs semantic oracle validation, compare observed and received time, inspect source-native/canonical records and exact lineage, review redelivery/conflict groups, rebuild a projection with explicit cutoffs, and inspect the manifest.
